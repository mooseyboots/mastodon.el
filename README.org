...
**** Do the following to obtain OAuth2 credentials:

1. Log in to your mastodon account
2. Go to "Preferences" under settings
3. Choose "Development"
4. Click on "New Application"
5. Give the app a name of your choice
6. Leave "Application website" empty
7. Insert `urn:ietf:wg:oauth:2.0:oob` in "Redirect URI"
8. In "Scopes" make sure `read`, `write` and `follow` are selected.
9. Click on "Save"

Now you should have client key(id) and client secret
